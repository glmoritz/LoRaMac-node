...
{
	sx126x_mod_params_lora_t ModulationParams; //straight from SX1262 configuration	
} __attribute__((packed));

#define LORA_RADIO_SET_PACKET_PARAMS (0x6666)
struct lora_set_packet_params
{
    sx126x_pkt_params_lora_t PacketParams; //straight from SX1262 configuration
} __attribute__((packed));
...
#define LORA_RADIO_SET_POWER (0xAAAA)
/*!
 * \brief Configures the radio power output
 *
 * \param [IN] unused - dummy payload
 *
 */
struct lora_set_power
{
    float Power_dbm;
} __attribute__((packed));
